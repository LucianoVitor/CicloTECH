    ...
    <>
      ...
      <section className="pt-16 pb-24 px-6 max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            <h1 className="text-5xl lg:text-7xl font-bold text-foreground leading-[0.9] tracking-tighter mb-8 font-data">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <Link to="/solicitar" className="px-8 py-4 bg-transparent text-foreground font-data text-sm uppercase tracking-widest border border-border hover:border-primary hover:text-primary transition-all">
      ...
      <Reveal>
        <section className="relative py-32 px-6 max-w-7xl mx-auto">
          <div className="mb-16">
            <h2 className="text-4xl font-bold text-foreground font-data tracking-tighter uppercase mb-4">Categorias de Doação</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((cat, i) => (
              <Reveal key={cat.title} delay={i * 0.06}>
                <div className="border border-border bg-card hover:border-primary/50 transition-all group overflow-hidden">
                  <div className="h-40 overflow-hidden bg-surface">
                    <img src={cat.image} alt={cat.title} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500" />
                  </div>
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      <h3 className="text-base font-bold text-foreground font-data tracking-tight">{cat.title}</h3>
                      <span className="text-[10px] font-data text-muted-foreground uppercase tracking-widest">{cat.count} itens</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <Link to="/solicitar" className="py-2 text-center text-[10px] font-data uppercase border border-border text-foreground hover:bg-secondary hover:text-secondary-foreground transition-all">
                        Solicitar
                      </Link>
                    </div>
                  </div>
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex items-end justify-between mb-16">
              <div>
                ...
                <h2 className="text-4xl font-bold text-foreground font-data tracking-tighter uppercase">Nosso Impacto</h2>
              </div>
              <div className="text-right hidden md:block">
                <p className="text-muted-foreground font-data text-[10px] uppercase">Data Refresh: 24.05.2026</p>
            ...
            <div className="grid md:grid-cols-3 gap-px bg-border border border-border">
              {impactData.map((item, i) => (
                <Reveal key={item.label} delay={i * 0.1}>
                  <div className="p-8 bg-card relative overflow-hidden group h-full">
                    ...
                    <div className="text-4xl font-bold font-data text-foreground mb-2 tabular-nums tracking-tighter">{item.value}</div>
